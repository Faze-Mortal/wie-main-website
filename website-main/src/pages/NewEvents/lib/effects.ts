...
function initCapsuleTilt() {
  const capsules = document.querySelectorAll('.event-page .event-card');
...
function initFanCards() {
  ...
  document.querySelectorAll('.event-page .deck-card')
);
...
function initSmoothScroll() {
  document
  .querySelectorAll('.event-page a[href^="#"]')
  .forEach((link) => {
...
export function useInteractiveEffects() {
  useEffect(() => {
    // Reset when the Events page opens through React Router
    fanCardsInitialized = false;
    particlesInitialized = false;

    // Wait until the Events page and all cards are mounted
    ...
    return () => {
      ...
      // Allow the effects to initialize when Events is opened again
      fanCardsInitialized = false;
      particlesInitialized = false;
    };
  }, []);
